...
// Licensed under the terms described in the LICENSE file in the root of this
// project.
//
...
bool DistributionInfo::CreateUser(std::wstring_view userName) {
  ...
  commandLine = L"/usr/sbin/usermod -aG wheel,adm ";
  commandLine += userName;
...
ULONG DistributionInfo::QueryUid(std::wstring_view userName) {
  ...
  if (CreatePipe(&readPipe, &writePipe, &sa, 0)) {
    ...
    int returnValue = 0;
    HANDLE child;
    HRESULT hr = g_wslApi.WslLaunch(command.c_str(), true,
                                    GetStdHandle(STD_INPUT_HANDLE), writePipe,
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        if (ReadFile(readPipe, buffer, (sizeof(buffer) - 1), &bytesRead,
                     nullptr)) {
