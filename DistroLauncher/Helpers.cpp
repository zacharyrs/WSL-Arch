...
namespace {
HRESULT FormatMessageHelperVa(DWORD messageId, va_list vaList,
                              std::wstring *message);
...
HRESULT FormatMessageHelperVa(DWORD messageId, va_list vaList,
  ...
  DWORD written = ::FormatMessageW(
      FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_ALLOCATE_BUFFER, nullptr,
      messageId, 0, (PWSTR)&buffer, 10, &vaList);
  *message = buffer;
  if (buffer != nullptr) {
    HeapFree(GetProcessHeap(), 0, buffer);
  }
